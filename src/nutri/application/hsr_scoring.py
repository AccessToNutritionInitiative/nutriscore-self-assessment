...
class HsrScoring: 
    ...
    def _score_protein(self, product: Product) -> int: 
        match product.category: 
            case ProductCategory.BEVERAGE_1: return 0
            case _: return self._protein_threshold(product.protein_g, Thresholds.PROTEIN)
            
    def _score_fibre(self, product: Product) -> int: 
        match product.category: 
            case ProductCategory.BEVERAGE_1 | ProductCategory.DAIRY_BEVERAGE_1D: return 0
            case _: return self._apply_threshold(product.fibre_g or 0, Thresholds.FIBRE)
    ...
    def _apply_threshold(self, value: float, threshold: Thresholds, right: bool = True) -> int: 
        ...
        Example: 
            ...
            [list[0], list[1]) -> 0
        """
        if right: return bisect.bisect_left(threshold, value) # for lower < x <= upper
        
        return bisect.bisect_right(threshold, value) # for lower <= x < upper
    
    def _protein_threshold(self, value: float, threshold: Thresholds,) -> int: 
