...
def test_bulk_returns_200():
    with CSV_FILE.open("rb") as f:
        response = client.post("/hsrs", files={"file": ("sample_hsr_case.csv", f, "text/csv")})
...
def test_bulk_rating():
    ...
    for exp, item in zip(expected, items, strict=True):
        assert exp["expected_score"] == item["final_score"]
        assert exp["expected_star_rating"] == item["star_rating"]
